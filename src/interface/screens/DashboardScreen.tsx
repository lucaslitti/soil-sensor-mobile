...
} from '../viewmodels/dashboardTypes';
...
export function DashboardScreen({ navigation }: Props) {
  ...
  const { devices: list, refresh: refreshDevices, remove } = useDeviceListViewModel();
  ...
  const refresh = async () => {
    ...
    setRefreshing(true);
    try {
      await refreshDevices(list.map(device => device.id));
    } finally {
      setRefreshing(false);
    }
  };
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl
            ...
            onRefresh={refresh}
            tintColor={colors.statusIdeas}
            colors={[colors.statusIdeas]}
          />
        }
      >
        {list.length === 0 ? (
